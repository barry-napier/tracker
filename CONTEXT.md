...
A machine check the orchestrator runs against a Run before a Ticket may leave Verifying. A cheap subset also runs at every Stage exit (ADR-0008): artifact lint scoped to what that Stage declares it owes, plus the repo suite — a failure feeds its findings back into the Stage's same live session, capped at 2 retries, and exhausting the cap is a Phase Death (gate-exhausted), not a bounce. In-phase results are recorded distinctly (`phase-gate:` prefix) and never join the bounce batch; greenness, freshness, and AC checks stay the Verifying battery's alone. Each gate results in pass, fail, or skip — skip means "not applicable," determined by facts (ticket type, repo config), never by the agent, and is distinct from pass. The battery is diagnostic: every gate and AC check runs even after a failure, and all failures bounce the Ticket together in one batch — each failed gate births a Follow-up Criterion, while a failed AC carries itself forward by resetting to pending on the next Run. The third failed cycle parks the Ticket in Human Review instead, flagged arrived-by-cap: it reached the veto point without passing gates (the crash cap parks with the same flag — see Phase Death). Agent-authored AC checks are gates: the agent writes the check, the orchestrator executes it — results cannot be self-reported.
...
What the workflow engine requires of every phase: run in a fresh provider session, receive context through the engine's fixed template variable set, and write `kb/<phase-name>.md` in the worktree before finishing. A phase completes only when the provider signals success and the contract file exists. A phase whose node has labeled outgoing edges must additionally declare its outcome in the contract file — one of the edge labels, passed in as template variables; a missing or unrecognized outcome is a Phase Death (contract breach). The engine only ever string-matches the declared outcome to pick the edge: routing is the phase's judgment, verification never is. The plan phase's contract additionally requires AC checks: one executable per machine-checkable Acceptance Criterion plus a manifest covering every pending AC (script or human-routing with reason).

### Phase Death

Any way a phase ends without fulfilling its contract: the provider crashing, exiting with failure, exiting cleanly without the contract file (hollow), breaching the extended contract (manifest, outcome declaration), failing its in-phase gate subset past the retry cap (gate-exhausted, ADR-0008), going silent past the orchestrator's patience, outliving the per-phase wall clock, or being orphaned by an app quit. Each mode is detected and audited distinctly, and all feed one policy: the phase is retried once; a second death ends the Run crashed. Crash is distinct from bounce — crash means the work didn't happen, so the Ticket returns to Todo with no new criteria and the next claim reuses the worktree as-is; bounce means the work was wrong, which only the Evidence Gate battery may say. Three crashed Runs park the Ticket in Human Review flagged arrived-by-cap, mirroring the bounce cap. At app launch any Run still marked running is an orphan: its evidence is persisted, then it is marked crashed and fed through the same policy.

### Done Sweep

The deliberate, batched reap of what a finished Ticket still holds on disk — its worktree and preview record. Done does not auto-destroy: the sweep is a human action on the Done column, and it reaps only Tickets satisfying the safety predicate — the PR verifiably merged on the remote and every piece of worktree evidence persisted to the artifact store. Anything unsafe is skipped and listed with the reason, never silently; each reap is an Audit Trail event. Parked and bounced Tickets are never candidates — they keep their worktrees indefinitely, a disk cost that buys inspectability. At app launch, reconciliation removes only true orphan directories: worktree folders no Ticket accounts for.
